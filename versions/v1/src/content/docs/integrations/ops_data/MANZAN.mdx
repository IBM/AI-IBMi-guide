...
2. *Destinations* are the locations you would like to send your data to. One option is to retrieve the data using your own custom ILE code, but in most cases you will want to use one of the [many supported destinations](https://theprez.github.io/Manzan/#/?id=where-can-i-send-these-events) based on your use case:
    ...
    * **General Purpose**
        * HTTP/HTTPS endpoints (REST APIs, webhooks, etc.)
        * File/Directory output

    > **Note:** For a complete list of available destinations and their implementation status, visit the [main documentation](https://theprez.github.io/Manzan/#/?id=where-can-i-send-these-events).
...
2. `data.ini`: This file will outline your *inputs* in a simple format. These will each have a unique ID and a type assigned based on if you are watching a stream file, message queue, etc. This is where you will also map your *inputs* to your *destinations*.
...
> **Learn More:** For detailed configuration options, format keys, and advanced features, see the [Configuration Documentation](https://theprez.github.io/Manzan/#/config/).

## Example 1: Sending Messages to Slack
...
* `format`: Defines the format in which the data should be sent in. If not specified, all information will be sent. In this case, you are only interested in the log message itself so you can use the special key `$FILE_DATA$` to get back just the file content.

> **Tip:** For all available format keys and filtering options, see the [Data Configuration Guide](https://theprez.github.io/Manzan/#/config/data).
...
* `[slack_out]`: Assigns `slack_out` as the unique ID for this *destination*. This is the same ID you used previously in the `destinations` field of `data.ini`.
* `type`: Specifies this *destination* to be of type `slack` which is the predefined type for Slack.
...
> **Learn More:** For all available destination types and their properties, see the [Destinations Configuration Guide](https://theprez.github.io/Manzan/#/config/dests).
...
## Example 2: Ingesting Logs into Grafana Loki
...
> **Prerequisites:** The steps below assume you have already created a Grafana Loki instance and have the credentials (`url`, `username`, and `password`) for it on hand.

### Configuring Manzan for Message Queue Monitoring
...
* `[loki_out]`: Assigns `loki_out` as the unique ID for this *destination*. This is the same ID you used previously in the `destinations` field of `data.ini`.
...
Now that you've seen how easy it is to configure *Manzan*, here are some resources to help you get started:

* **[Installation Guide](https://theprez.github.io/Manzan/#/install)** - Learn how to install and deploy Manzan on your IBM i system
* **[Configuration Examples](https://theprez.github.io/Manzan/#/config/examples/)** - Explore more detailed examples for various destinations including Prometheus, Elasticsearch, Splunk, and more
* **[Data Sources](https://theprez.github.io/Manzan/#/config/data)** - Learn about all available input types including SQL events, job logs, audit logs, and HTTP endpoints
* **[Message Formatting](https://theprez.github.io/Manzan/#/config/format)** - Master the art of formatting your messages with special keys and custom templates
* **[Contributing](https://theprez.github.io/Manzan/#/contributing)** - Want to add a new destination or feature? Learn how to contribute to the project

Ready to integrate IBM i with your modern IT infrastructure? [Get started with Manzan today!](https://theprez.github.io/Manzan/#/)